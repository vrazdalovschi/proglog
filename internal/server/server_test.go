...
func setupTest(t *testing.T, fn func()) (rootClient api.LogClient, nobodyClient api.LogClient, cfg *Config, teardown func()) {
	t.Helper()
...
func testUnauthorized(t *testing.T, _, nobodyClient api.LogClient, _ *Config) {
	ctx := context.Background()
	produce, err := nobodyClient.Produce(context.Background(),
		&api.ProduceRequest{
			Record: &api.Record{
				Value: []byte("hello world"),
			},
